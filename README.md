...
- **ノート写真確認** - 生徒が撮影した実験ノート写真の管理
  ...
  - ZIP形式での一括ダウンロード
  - 写真の削除機能
- **エラーモニタリング** - 学習中のシステムエラーをリアルタイム監視
  - クライアント側のJavaScript エラー自動キャプチャ
  - APIエラー、Promise rejection エラーの記録
  - 日付・クラスでフィルタリング可能なダッシュボード
  - Firestore（本番）またはJSON（ローカル）で自動保存
...
## 🆕 最新アップデート（2025年11月14日）

### v1.4 - 会話復帰機能と堅牢化
- **会話履歴復帰機能** - ページリロードや一時的な接続断後も会話を再開可能
  - セッションとFirestoreデータの二重管理で信頼性向上
  - 予想段階・考察段階の両方で対応
  - ユーザー入力フィールドが削除される問題を修正
  
- **単元混在防止** - 異なる単元に切り替えた際のセッションクリア
  - `/prediction`と`/reflection`の両方で単元チェック
  - 同じブラウザから異なる単元への切り替えで無意識の混在を防止
  
- **ノート写真表示機能の修正** - 教師ダッシュボードでノート写真が正しく表示されるようサーバー側の修正
  - `/logs/note_photos/<path:path>`ルートで静的ファイル配信
  - Flask send_from_directoryで確実なファイル提供
  
- **ノート撮影ボタンの無効化** - 「ノート撮影は1回のセッション」の仕様に合わせてボタンをdisabled化
  - クリック不可にするが、UI上は表示維持
  
- **セッションデータ渡し方の修正** - テンプレートでのセッションアクセス方法を改善
  - `render_template()`で明示的に`conversation_history`や`reflection_conversation_history`を渡す
  - Jinja2テンプレートで`{{ session.get() }}`が解決されない問題を根本解決
