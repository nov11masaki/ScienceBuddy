...
- **理科のお助けチャット** 🆕 - Gemini API使用の学習支援チャットボット
  - 理科の勉強でわからないことを自由に質問
  - 小学生にわかる平易な言葉で説明
  - 答えを直接教えず、考えるヒントを提供
  - いつでも気軽に利用可能
...
GEMINI_API_KEY=your_actual_gemini_api_key_here  # チャットボット機能用（オプション）
```

**注意**: 
- OPENAI_API_KEYは必須（学習支援機能で使用）
- GEMINI_API_KEYはオプション（チャットボット機能で使用、未設定の場合はOpenAI APIにフォールバック）
